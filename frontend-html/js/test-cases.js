...
async function loadTestCases() {
    try {
        ...
        // Get API URL
        const apiBaseUrl = getApiBaseUrl();
        const url = `${apiBaseUrl}/api/test-cases`;
        ...
        if (data.is_empty || (data.success === false && data.message && data.message.includes('数据库为空'))) {
            showEmptyState();
            testCases = [];
            return;
        }
        ...
        // Check data format
        if (!data.testCases && !data.data) {
            throw new Error('API response format is incorrect, missing testCases or data field');
        }
